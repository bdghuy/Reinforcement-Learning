...
class Critic(RLNN):
    def __init__(self, state_dim, action_dim, max_action, beta, args):
    ...
    def forward(self, x, u):
        ...
        if not self.layer_norm:
            x = F.leaky_relu(self.l1(sa))
            x = F.leaky_relu(self.l2(x))
            x = self.l3(x)

        else:
            x = F.leaky_relu(self.n1(self.l1(sa)))
    ...
    def update(self, memory, batch_size, actor_t, critic_t):
        ...
        with torch.no_grad():
            n_actions = actor_t(n_states)
            
            noise = torch.randn(
				(actions.shape[0], self.num_noise_samples, actions.shape[1]), 
				dtype=actions.dtype, layout=actions.layout, device=actions.device
			)
            noise = noise * self.policy_noise
            noise = noise.clamp(-self.noise_clip, self.noise_clip)
            
            n_actions = torch.unsqueeze(n_actions, 1)
            n_actions = (n_actions + noise).clamp(-self.max_action, self.max_action)
            
            n_states = torch.unsqueeze(n_states, 1)
            n_states = n_states.repeat((1, self.num_noise_samples, 1))
            
            next_Q = critic_t(n_states, n_actions)
            next_Q = torch.squeeze(next_Q, 2)
            
            softmax_next_Q = self.softmax_operator(next_Q)

            target_Q = rewards + (1 - dones) * self.discount * softmax_next_Q
...
class CriticSD3(RLNN):
    def __init__(self, state_dim, action_dim, max_action, beta, args):
        super(CriticSD3, self).__init__(state_dim, action_dim, 1)
    ...
    def forward(self, x, u):
        ...
        if not self.layer_norm:
            x1 = F.leaky_relu(self.l1(sa))
            x1 = F.leaky_relu(self.l2(x1))
            x1 = self.l3(x1)

        else:
            x1 = F.leaky_relu(self.n1(self.l1(sa)))
            x1 = F.leaky_relu(self.n2(self.l2(x1)))
            x1 = self.l3(x1)

        if not self.layer_norm:
            x2 = F.leaky_relu(self.l4(sa))
            x2 = F.leaky_relu(self.l5(x2))
            x2 = self.l6(x2)

        else:
            x2 = F.leaky_relu(self.n4(self.l4(sa)))
    ...
    def update(self, memory, batch_size, actor_t, critic_t):
        ...
        with torch.no_grad():
            ...
            next_Q1, next_Q2 = critic_t(n_states, n_actions)
...
if __name__ == "__main__":
    ...
    # SD3 parameters
    parser.add_argument('--use_sd3', dest='use_sd3', action='store_true')
    ...
    beta = env_beta_map[args.env.split('-')[0]]
    # critic
    if args.use_sd3: #SD3
        critic = CriticSD3(state_dim, action_dim, max_action, beta, args)
        critic_t = CriticSD3(state_dim, action_dim, max_action, beta, args)
        critic_t.load_state_dict(critic.state_dict())

    else: #SD2
        critic = Critic(state_dim, action_dim, max_action, beta, args)
        critic_t = Critic(state_dim, action_dim, max_action, beta, args)
        critic_t.load_state_dict(critic.state_dict())
